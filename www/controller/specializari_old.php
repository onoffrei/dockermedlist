...
function specializari_inputedit_rs($p_arr = array()){
	...
	$ret['specializari_user_spitale_tags_a'] = cs('specializari_user_spitale/tags_a', array("user"=>$p_arr['id'],"spital"=>$p_arr['spital']));
	if (!isset($ret['specializari_user_spitale_tags_a']['success']) || ($ret['specializari_user_spitale_tags_a']['success'] != true)) return $ret;
	
	$ret['specializari_breadcrumb'] = cs('specializari/breadcrumb', array('catid'=>$ret['catid'],'callback'=>'specializari_inputchoose_onclick'));
	if (!isset($ret['specializari_breadcrumb']['success']) ||  ($ret['specializari_breadcrumb']['success'] != true)) return $ret;

	$ret['specializari_list_a'] = cs('specializari/list_a',array('catid'=>$ret['catid'],'callback'=>'specializari_inputchoose_onclick'));
	...
	?><div id="specializari_inputedit_modal" class="modal fade" tabindex="-1" role="dialog">
		...
		specializari_inputedit_saveonclick = function(){
			//var formData = new FormData(document.querySelector('#specializari_inputedit_form'))
			//var formDataJson = {};
			//formData.forEach(function(value, key){formDataJson[key] = value;});
			//console.log(formDataJson)
			var specializari = [];
			$('#specializari_inputedit_form input[type=checkbox]').map((i,e)=>{
				var specializare = {};
				//if (typeof(e.dataset.sdid) != 'undefined'){
					specializare.id = parseInt(e.name)
					specializare.checked = e.checked
					specializare.sdid = parseInt(e.dataset.sdid)
					specializari[specializari.length] = specializare
				//}
			}).get()
			...
			}).then((specializari_user_spitale_userset)=>{
				$("#specializari_inputedit_modal").modal('hide')
			})
		}
		specializari_inputedit_onclick = function(catid){
			cs('specializari/breadcrumb',{catid:catid,callback:'specializari_inputedit_onclick'}).then(function(specializari_breadcrumb){
			...
			//cs('specializari_user_spitale/tags_a',{user:<?php echo $p_arr['id']?>,spital:<?php echo $p_arr['spital']?>,callback:'specializari_inputedit_onclick'}).then(function(tags_a){
				//$("#specializari_inputedit_modal div.specializari_tags").html(tags_a.resp.html)
			//})
			cs('specializari/list_a',{catid:catid,callback:'specializari_inputedit_onclick'}).then((specializari_list_a)=>{
				if (specializari_breadcrumb_nodearr.length == 0){
					...
					cs('specializari_user_spitale/grid',{filters:{rules:[
						{field:'spital',op:'eq',data:<?php echo $p_arr['spital']?>},
						{field:'user',op:'eq',data:<?php echo $p_arr['id']?>}
					]}}).then((specializari_user_spitale_grid)=>{
						if (typeof(specializari_user_spitale_grid.success) != 'undefined' && (specializari_user_spitale_grid.success == true)){
							var specializareids = specializari_user_spitale_grid.resp.rows.map((specializare)=>{
								return parseInt(specializare.specializare)
							})
							console.log(specializareids)
							...
							$("#specializari_inputedit_modal div.modal-body div.list-group").append(
								specializari_list_a.resp.specializari_grid.resp.rows.map((d,i)=>{
									var checkbox_options = {
										type:'checkbox',
										name:d.id,
										'data-sdid':'',
									}
									var found = specializareids.indexOf(parseInt(d.id))
									if (found >= 0) {
										checkbox_options.checked = 'checked';
										checkbox_options['data-sdid'] = specializari_user_spitale_grid.resp.rows[found].id;
									}
		...
		specializari_inputedit_onclick(<?php echo $ret['catid']?>)
	</script><?php 
	$ret['resp']['html'] = ob_get_contents();
	ob_end_clean();
	$ret['success'] = true;
...
function specializari_inputchoose_rs($p_arr = array()){
	...
	if (isset($p_arr['active'])) { 
		$options['catid'] = (count($ret['specializari_breadcrumb']['resp']['nodearr']) > 1)?$ret['specializari_breadcrumb']['resp']['nodearr'][1]:0;
...
function specializari_breadcrumb($p_arr = array()){
	...
	?> <ol id="specializari_breadcrumb" class="breadcrumb"> <?php 
			if ($nodearr_count == 0){
				echo '<li class="active">Specializari</li>';
			}else{
				echo '<li><a href="javascript:void(0)" onclick="' . $p_arr['callback'] . '(0)">Specializari</a></li>';
			}
			for ($i = $nodearr_count - 1; $i >= 0; $i-- ){
				if ($i == 0){
					echo '<li class="active">' . $ret['resp']['nodearr'][$i]['denumire'] . '</li>';						
				}else{
					echo '<li><a href="javascript:void(0)" onclick="' . $p_arr['callback'] . '(' . $ret['resp']['nodearr'][$i]['id'] . ')">' . $ret['resp']['nodearr'][$i]['denumire'] . '</a></li>';						
				}
			}
	?> </ol> <?php
	$ret['resp']['html'] = ob_get_contents();

	...
	foreach($ret['resp']['cauta_list']['resp']['rows'] as $ret['item']){
		...
		<div class="row listitem">
			...
			<div class="col-sm-6 col-md-7">
				<?php 
				if ($ret['item']->planificare_start == '2222-12-22 22:22:22'){
				?>
				<div class="row">
					<div class="col-xs-12">
						<a href="<?php 
							echo cs_url . '/doctori/' . $ret['localitate_uri'] . '/' . $ret['item']->spitale_uri . '/' . $ret['item']->doctor_uri
							?>" class="interval_item">
							Pagina de profil pentru <?php echo $ret['item']->doctor_nume ;?>
						</a>
					</div>
				</div>
				<?php 
				} else if (strtotime(date('Y-m-d',strtotime($ret['item']->planificare_start))) > strtotime(date('Y-m-d',strtotime($ret['resp']['cauta_list']['date'])))){
				?>
				<div class="row">
					<div class="col-xs-12">
						<a href="javascript:void(0)" class="interval_item" onclick="programari('<?php 
							echo $ret['localitate_uri'] . '/' . $ret['item']->spitale_uri . '/' . $ret['item']->doctor_uri 
						...
						<?php echo 'Programare online începând cu data de ' . date('d.m.Y',strtotime($ret['item']->planificare_start));?>
						</a>
					</div>
				</div>
				<?php	
...
function cauta_list($p_arr = array()){
	...
	$ret['cauta_sql'] .= "    spitale.id spitale_id";
	...
	$ret['cauta_sql'] .= " 	, users.id as doctor_id";
	...
	$ret['cauta_sql'] .= " 		FROM logs";
	$ret['cauta_sql'] .= " 		WHERE ";
	$ret['cauta_sql'] .= " 			logs.alttype = 2  ";
	$ret['cauta_sql'] .= " 			AND logs.altid = users.id  ";
	$ret['cauta_sql'] .= " 	) as vizualizari";
	...
	$ret['cauta_sql'] .= " 	),'2222-12-22 22:22:22') as planificare_start";
	$ret['cauta_sql'] .= " FROM specializari_user_spitale";
	$ret['cauta_sql'] .= " LEFT JOIN planificare planificare_tmain on (specializari_user_spitale.spital = planificare_tmain.spital) AND (specializari_user_spitale.user = planificare_tmain.doctor)";
	$ret['cauta_sql'] .= " LEFT JOIN users on users.id = specializari_user_spitale.user";
	...
	$ret['cauta_sql'] .= " 	specializari_user_spitale.spital ";
	$ret['cauta_sql'] .= " 	, specializari_user_spitale.user ";
	//$ret['cauta_sql'] .= " HAVING planificare_start IS NOT NULL ";
	//$ret['cauta_sql'] .=  " ORDER BY planificare_start ASC, vizualizari DESC, doctor_id ASC";
	$ret['cauta_sql'] .=  " ORDER BY planificare_start ASC, spitale_id DESC, doctor_id ASC";
	$ret['cauta_sql'] .=  "  __limit__";

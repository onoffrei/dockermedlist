...
if ($is_success){
	...
	$unitateuri = $matches[0][1];
}
if ($is_success){
	
	$spitale_get = cs('spitale/get', array("filters"=>array("rules"=>array(
		array("field"=>"uri","op"=>"eq","data"=>$unitateuri)
	))));
...
if ($is_success){
	...
	if (!isset($localitati_spitale['success']) || ($localitati_spitale['success'] == false)) {$is_success = false;}
...
<?php 
function header_ob(){ 
	global $is_success, $spitale_get;
	$ret = array('success'=>false, 'resp'=>array());
	ob_start();
	if ($is_success){
	?>
	<title>Statistica unitate <?php echo $spitale_get['nume']?> - MedList.ro</title>
...
<div class="container-fluid">
	<div class="row">
		<div class="col-xs-12">
			<h4>
				Statistica pentru unitatea: 
				<a href="<?php 
					echo cs_url . '/doctori/' . $localitate_uri . '/' . $spitale_get['uri']
				?>">
...
?>
	...
	<script>
		...
		cs('logs/graphdata',{alttype:1,altid:<?php echo $spitale_get['id']; ?>}).then(function(d){
